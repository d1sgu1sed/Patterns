...
class TestCompanyModel(unittest.TestCase):
    ...
    def test_load_createmodel_companymodel(self):
        ...
        # проверка
        assert result
        assert load_config.model.settings.name != ''
    ...
    def test_class_equals(self):
        ...
        # проверка
        assert load_config1.model == load_config2.model
    ...
    def test_loading_dict_settings(self):
        ...
        settings = load_config.model.settings
        raw_data = {
            "name": "Рога и копыта",
            "INN": "129741029322",
            "account": "12309184024",
        ...
        for key in raw_data.keys():
            assert raw_data[key] == getattr(settings, key)
    
    def test_loading_dict_settings_from_uncommon_config(self):
        # подготовка
        filename = '/home/ivan/config.json'
        load_config: SettingsManager = SettingsManager(filename)
        settings: Settings
        # действие
        ...
        # assert getattr(settings, 'name') == raw_data["name"] 
        for key in raw_data.keys():
            assert raw_data[key] == getattr(settings, key)
