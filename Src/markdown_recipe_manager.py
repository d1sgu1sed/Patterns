...
class markdown_recipe_manager:
    ...
    def create(filename: str, recipe: recipe_model):
        validator.validate(filename, str)
        if filename in markdown_recipe_manager._instances.keys():
            return markdown_recipe_manager._instances[filename]
        validator.validate(recipe, recipe_model)
        item = markdown_recipe_manager(filename, recipe)
        markdown_recipe_manager._instances[filename] = item
        return item
